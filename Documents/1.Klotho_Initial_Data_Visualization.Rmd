...
fdr.thresh <- 0.05 #fdr for differentially expressed genes
paper.stats <- list("fdr.thresh" = fdr.thresh, "min.term.size" = min.term.size) #initialize an object for keeping track of stats that go in the paper
klotho.stats <- list() #this object is for the effects of each factor on Klotho expression. We run these in the full group
                            #so it needs to be saved separately from the stats only run in the 12-month group
...
use.carrier.status <- FALSE #if true, fits models with the ordered factor FC > WT > VS
                          #if false, fits models with the ordered factor FC/FC > FC/WT > WT/WT > WT/VS > VS/VS
...
get.enrichments = TRUE #this allows us to turn of gprofiler code when gprofiler is down or we are offline
```
...
needed.libraries <- c("synapser", "pheatmap", "DESeq2", "DT", "vioplot", "RColorBrewer",
  ...
  "wordcloud2", "easyPubMed", "tm", "corto")
...
condensed.table[which(condensed.table[,"sex_climb"] == "male"),"sex_climb"] <- "Male"
...
write.table(condensed.table, file.path(results.dir, "mouse.info.csv"), sep = ",", quote = FALSE, row.names = FALSE)
```
...
#sapply(count.tables, sum)
#sum(sapply(count.tables, sum))
```
...
```{r recount_groups}
sexes <- unique(condensed.table[,"sex_climb"])
ages <- sort(as.numeric(unique(condensed.table[,"age_batch"])))
genotypes <- unique(condensed.table[,"climb_geno"])[c(2,5, 4, 1, 3)]
...
for(s in 1:length(sexes)){
    ...
    for(g in 1:length(genotypes)){
      genotype.idx <- grep(genotypes[g], condensed.table[,"climb_geno"])
      for(ag in 1:length(ages)){
        age.idx <- which(condensed.table[,"age_batch"] == ages[ag])
        num.in.group <- length(Reduce("intersect", list(sex.idx, genotype.idx, age.idx)))
        by.age[g,ag] <- num.in.group
      }
    }
...
saveRDS(assay(vsd), file.path(processed.data.dir, "Stabilized_Counts.RDS"))
```
...
gene.counts <- sub.count[gene.idx,]
#gene.counts <- assay(vsd)[gene.id,]
#boxplot(as.numeric(gene.counts))
...
#scale expression so we can compare differences across genes
scaled.expr <- t(apply(norm.expr, 1, scale))
#scaled.expr <- norm.expr
...
if(age.varies && use.carrier.status){ #report Klotho stats in paper for whole group
  pdf(here("Results", "for_paper", 
    paste0("Klotho_expression_by_genotype_", basename(results.dir), "_months.pdf")), 
    width = 5.5, height = 5)
    par(bg = NA)
  geno.stats <- plot_tx_with_factor(expr.mat = scaled.expr, covar.table = covar.mat, 
    tx_name = tx.id, "ordered_geno", group.cols = geno.cols, 
    tx_label = paste0("Kl Transcript", 1:length(tx.id)), 
    ylab = "Expression (A.U.)", order.by.mean = FALSE, cex.labels = 1.5,
    cex.axis = 1.2, 
    label.shift = -0.7, stat.x = 0.1, stat.y = 0.25, stat.y.spread = 0.18)
  dev.off()

  klotho.stats$geno_effect_on_Kl <- geno.stats
}
```
...
plot_tx_with_factor(expr.mat = scaled.expr, covar.table = covar.mat, 
  tx_name = tx.id, factor_name = "sex_ge", tx_label = paste0("Kl Transcript", 1:length(tx.id)), 
  ylab = "Expression (A.U.)", stat.x = 0.02, stat.y = 0.2, autoflip.stat.y = FALSE)
...
if(age.varies && use.carrier.status){ #report Klotho stats in paper for whole group
  pdf(here("Results", "for_paper", 
    ...
    width = 5, height = 5)
    par(bg = NA)
  sex.stats <- plot_tx_with_factor(expr.mat = scaled.expr, covar.table = covar.mat, 
    tx_name = tx.id, factor_name = "sex_ge", tx_label = paste0("Kl Transcript", 1:length(tx.id)), 
    ylab = "Expression (A.U.)", cex.labels = 1.5, cex.axis = 1.2, stat.x = 0.35, stat.y = 0.2)
  dev.off()

  #should we report sex effects for group overall, or just 12-month old mice?
  klotho.stats$sex_effect_on_Kl <- sex.stats
}
```
...
if(age.varies && use.carrier.status){
  #this is only run if we are looking at all ages, so we need to write 
  #out the results separately

  pdf(here("Results", "for_paper", 
    paste0("Klotho_expression_by_age_", basename(results.dir), "_months.pdf")), 
    width = 5, height = 5)
    par(bg = NA)
  age.stats <- plot_tx_with_factor(expr.mat = scaled.expr, covar.table = covar.mat, 
    tx_name = tx.id, factor_name = "age_batch",
    tx_label = paste0("Kl Transcript", 1:length(tx.id)), cex.axis = 1.2,
    ylab = "Expression (A.U.)", cex.label = 1.5, stat.x = 0.37, stat.y = 0.2)
  dev.off()

  klotho.stats$age_effect_on_Kl <- age.stats
  saveRDS(klotho.stats, here("Results", "for_paper", "paper_stats_Klotho_effects.RDS"))
}
```
...
test_full_model <- function(expr.mat, covar.mat){
  ...
  if(!file.exists(full.model.file)){
    ...
    effect.p <- effect.beta <- matrix(NA, nrow = ncol(adj.expr), ncol = num.factors)
    colnames(effect.p) <- colnames(effect.beta) <- factor.names[1:num.factors]
    ...
    for(i in 1:ncol(adj.expr)){
      ...
      linear.idx <- sapply(colnames(effect.beta), 
        function(x) which(gsub("Male", "", 
        gsub(".L", "", names(coef(diff.test)), fixed = TRUE)) == x))
      effect.beta[i,] <- coef(diff.test)[linear.idx]
      #coef(diff.test)
    }
    rownames(effect.p) <- rownames(effect.beta) <- names(model.r2) <- colnames(adj.expr)
    #par(mar = c(12,4,4,2)); boxplot(-log10(effect.p), las = 2, ylab = "-log10(p)")
    #plot.decomp(t(-log10(effect.p)), label.points = TRUE)
    result <- list("beta" = effect.beta, "p_values" = effect.p, "R2" = model.r2)
...
model.effects <- full.model.result$beta
#dim(effect.p); length(model.r2); dim(model.effects)
...
model.r2 <- model.r2[-c(big.sex.effect, ide.effect)]
model.effects <- model.effects[-c(big.sex.effect, ide.effect),]
#dim(effect.p); length(model.r2); dim(model.effects)
```
...
#run with each age group. The results are written to the paper
#results directory so we can collect them easily
...
#run this workflow for age = 4 months, age = 12 months, and age = all 
#to generate all files for the table in the paper.
if(length(de.files) > 2){
  ...
  #remove the Ide columns. We don't care about those
  ide.col <- grep("Ide", colnames(count.table))
  #convert "genotype" to "haplotype" to match paper convention
  colnames(count.table) <- gsub("genotype", "haplotype", colnames(count.table))
  #convert Klotho to Kl
  colnames(count.table) <- gsub("Klotho", "Kl", colnames(count.table))
  #change "group" to "cohort"
  colnames(count.table)[1] <- "Cohort"
  #change rownames to be more readable
  count.table[,1] <- c("12 months", "4 months", "All")
  #reorder rows to be 4, 12, all
  write.table(count.table[c(2,1,3),-ide.col], here("Results", "for_paper", "num_de_table.txt"), 
    row.names = FALSE, na = "--", quote = FALSE, sep = "\t")

  #cd to ~/Documents/git_repositories/LaTeX-Table-Generator
  #run python3 table_generator.py
  #path to file: /Users/atyler/Documents/Projects/Klotho/klotho_analysis/Results/for_paper/num_de_table.txt
  #output name: de_table
  #move the result from Outputs to the for_paper directory
  #mv Outputs/de_table.tex /Users/atyler/Documents/Projects/Klotho/klotho_analysis/Results/for_paper/
  #open the file and paste the table into the manuscript document
  #paste the result into LaTeXit to test if needed.
  #add row and column separators and make all row and column labels bold

  #add stats to our paper stats object
  paper.stats$deg_12 <- as.numeric(count.table["age_batch_12_gene","Kl haplotype"])
  paper.stats$deg_4 <- as.numeric(count.table["age_batch_4_gene","Kl haplotype"])
}

#write out paper stats
#this directory has a symbolic link to the paper directory, so we can 
#pull from it directly.
saveRDS(paper.stats, here("Results", "for_paper", "paper_stats_DEG.RDS"))
```
...
cluster.decomp <- plot.decomp(mean.sig.expr, plot.results = FALSE)
clustering <- rep(1, nrow(mean.sig.expr))
clustering[which(cluster.decomp$u[,1] < 0)] <- 2
col.order <- order(cluster.decomp$u[,1], decreasing = TRUE)

annot.col <- data.frame(as.factor(clustering))
colnames(annot.col) <- "cluster"
rownames(annot.col) <- rownames(mean.sig.expr)
...
gene.stat.table <- cbind(rownames(mean.sig.expr), gene.names, 
  model.effects[sig.geno.idx,"ordered_geno"], effect.p[sig.geno.idx,"ordered_geno"], 
  factor.fdr[sig.geno.idx,"ordered_geno"],
  mean.sig.expr, annot.col)[order(annot.col[,1]),]
colnames(gene.stat.table) <- c("geneID", "geneName", "genotype_beta",
  "genotype_p", "genotype_fdr", colnames(mean.sig.expr), "cluster")

write.table(gene.stat.table, 
  file.path(results.dir, paste0("DEG_clusters_", carrier.text, ".txt")), 
  sep = "\t", quote = FALSE)


if(final.params){
  write_table_with_header(gene.stat.table, file.path(here("Results", "for_paper", "Fig_2B_Data.txt")),
    header = "Differentially expressed genes in 12-month old mice with varying Kl haplotypes. Differential expression statistics are reported for each gene along with mean expression values for each haplotype and the cluster assigned to each gene. These data correspond to 2B.")
}
```

If we are looking at all genotypes separately, the following code compares
the additive and dominant models to justify grouping the carriers together.
That is, we group the WT/VS and the VS/VS together as "VS carriers" and the 
WT/FC and FC/FC together as "FC carriers" and compare these two groups to 
the WT/WT animals.


```{r dominance, warning = FALSE, fig.width = 10, fig.height = 5}

if(!use.carrier.status){

  library(locfit)

  deg.decomp <- plot.decomp(t(sig.expr), plot.results = FALSE)
  deg.pc <- deg.decomp$u[,1]

  add_dom <- function(y){

    carrier.factor <- rep("WT", length(geno_factor))
    carrier.factor[grep("VS", geno_factor)] <- "VS"
    carrier.factor[grep("FC", geno_factor)] <- "FC"
    dom_coding <- ordered(carrier.factor, levels = c("FC", "WT", "VS"))

    add.levels <- levels(geno_factor)
    add.num <- sapply(geno_factor, function(x) which(add.levels == x)-3)

    dom.levels <- levels(dom_coding)
    dom.num <- sapply(dom_coding, function(x) which(dom.levels == x)-2)

    #add.model <- lm(y~geno_factor) 
    #dom.model <- lm(y~dom_coding)

    add.model <- lm(y~add.num) 
    dom.model <- lm(y~dom.num)

    geno.add.r2 <- summary(add.model)$adj.r.squared
    geno.add.p <- coef(summary(add.model))[2,"Pr(>|t|)"]
    geno.add.aic <- aic(add.model)

    geno.dom.r2 <- summary(dom.model)$adj.r.squared
    geno.dom.p <- coef(summary(dom.model))[2,"Pr(>|t|)"]
    geno.dom.aic <- aic(dom.model)

    result <- cbind(c(geno.add.r2, geno.add.p, geno.add.aic["aic"]), c(geno.dom.r2, geno.dom.p, geno.dom.aic["aic"]))
    colnames(result) <- c("Additive", "Dominant")
    rownames(result) <- c("R2", "p", "AIC")
    return(result)
  }


  sig.deg.comparison <- lapply(1:nrow(sig.expr), function(x) add_dom(sig.expr[x,]))
  comp.r2 <- t(sapply(sig.deg.comparison, function(x) x["R2",]))
  comp.p <- t(sapply(sig.deg.comparison, function(x) x["p",]))
  comp.AIC <- t(sapply(sig.deg.comparison, function(x) x["AIC",]))
  
  pc.model.comparison <- add_dom(deg.pc)
  
  pdf("~/Desktop/Additive_vs_Dominant.pdf", width = 9, height = 12)
  par(mfrow = c(4,3))
  barplot_with_num(signif(pc.model.comparison["R2",], 2), ylab = "R2", 
    main = "Variance Explained", ylim = c(0, 0.25), text.shift = 0.13)
  barplot_with_num(signif(-log10(pc.model.comparison["p",]), 2), 
    ylab = "-log10(p)", main = "-log10(p)", ylim = c(0, 8), text.shift = 0.13)
  barplot_with_num(signif(pc.model.comparison["AIC",], 2), 
    ylab = "AIC", main = "AIC", ylim = c(0, 12), text.shift = 0.09)

  boxplot(comp.r2, ylab = bquote(italic(R)^2), main = "Variance Explained")
  draw.rectangle(0.42, 1.2, 0.22, 0.25, fill = "white")
  test.diff <- t.test(comp.r2[,1], comp.r2[,2])
  test.p <- threshold_p(p = test.diff$p.value, return.text = TRUE, sci.not = TRUE)
  text(x = 0.55, y = 0.235, labels = test.p[1])
  text(x = 0.9, y = 0.237, labels = test.p[2])
  boxplot(-log10(comp.p), ylab = "-log10(p)", main = "-log10(p)")
  boxplot(comp.AIC, ylab = "AIC", main = "AIC")

  plot(comp.r2, main = "Variance Explained", xlab = paste("Additive", bquote(italic(R^2))), 
    ylab = "Dominant R2")
    abline(0,1, col = "red")
  plot(-log10(comp.p), main = "-log10(p)", xlab = "Additive -log10(p)", ylab = "Dominant -log10(p)")
    abline(0,1, col = "red")
  plot(comp.AIC, main = "AIC", xlab = "Additive AIC", ylab = "Dominant AIC")
    abline(0,1, col = "red")

  plot.text("")
  plot.text("")
  
  rel.likelihood <- sapply(pc.model.comparison["AIC",], function(x) exp((min(pc.model.comparison["AIC",])-x)/2))
  barplot(rel.likelihood, ylab = "Relative Likelihood")
...
```{r deg_fig_paper}
cluster.id <- clustering

if(final.params){
  
  if(use.carrier.status){
    png(here("Results", "for_paper", "deg_clusters.png"), width = 7, height = 2,  units = "in", res = 300)
    y.nudge = 0.15; x.nudge.left = 0.04; x.nudge.right = 0.03; row.text.shift = 0.045; label.y = 0.15
  }else{
    #only run by hand
    png(here("Results", "for_paper", "deg_clusters_all_geno.png"), width = 7, height = 3,  units = "in", res = 300)
    y.nudge = 0.06; x.nudge.left = 0.09; x.nudge.right = 0.03; row.text.shift = 0.07; label.y = 0.12
  }
  ...
  imageWithText(t(scaled.mat), show.text = FALSE, split.at.vals = TRUE,
    col.scale = list(heatmap.neg.col, heatmap.pos.col), grad.dir = "ends",
    col.names = NULL, row.text.shift = row.text.shift, row.text.adj = 0.5)
  plot.dim <- par("usr")
  ...
  label.y <- (plot.dim[4]+(plot.height*label.y))
  #group.summ
  ...
  y.vals <- 1:length(alleles)
  y.shift <- plot.height*y.nudge
  label.xmin <- plot.dim[1] - (plot.width*x.nudge.left)
  label.xmax <- plot.dim[1] + (plot.width*x.nudge.right)
  for(i in 1:length(y.vals)){
    draw.rectangle(label.xmin, label.xmax, (y.vals[i] - y.shift), (y.vals[i] + y.shift),
      border = geno.cols[match(rev(alleles)[i], names(geno.cols))], 
      lwd = 3)
  }

  #add scale bar
  par(mar = c(2,4,4,1))
  imageWithTextColorbar(scaled.mat, split.at.vals = TRUE, 
  col.scale = list(heatmap.neg.col, heatmap.pos.col), cex = 1, 
  grad.dir = "ends", bar.lwd = 3, round.nearest = 0.1)
  dev.off()


  #more stats for grouping of haplotypes
  vs.mat <- scaled.mat[,grep("VS", colnames(scaled.mat))]
  fc.mat <- scaled.mat[,grep("FC", colnames(scaled.mat))]
  mean.vs <- rowMeans(vs.mat)
  mean.fc <- rowMeans(fc.mat)
  upreg <- which(mean.vs - mean.fc > 0)
  downreg <- which(mean.vs - mean.fc < 0)
  boxplot(cbind(vs.mat[upreg,], fc.mat[upreg,]))
  boxplot(cbind(vs.mat[downreg,], fc.mat[downreg,]))
...
if(get.enrichments){
  ...
  group.summ <- sapply(lapply(group.enrich, function(x) summarize_enrichment(x, 
    num.words = 3, max.term.size = 3000)), function(y) paste(y, collapse = ", "))
}
```
...
The following plot shows the mean gene expression across genotypes
along with automatically generated enrichment summaries. These 
are for keeping track of which cluster has which enrichment. 

```{r deg_fig_report, fig.width = 10, fig.height = 2.5}
cluster.id <- clustering
...
c2.mid <- mean(which(cluster.id[col.order] == 2))
label.y <- (plot.dim[4]+(plot.height*label.y))
text(x = c1.mid, y = label.y, labels = group.summ[1])
text(x = c2.mid, y = label.y, labels = group.summ[2])
...
#png("~/Desktop/geno_cor.png", width = 4, height = 3, units = "in", res = 300); par(bg = NA)
pheatmap(cor(mean.sig.expr), display_numbers = TRUE, cluster_rows = FALSE, cluster_cols = FALSE)
#dev.off()
```
...
cluster.id <- expr.clust$clustering
  ...
  imageWithTextColorbar(mean.sig.expr, split.at.vals = TRUE, 
    col.scale = list(heatmap.neg.col, heatmap.pos.col), cex = 1, bar.lwd = 3, 
    grad.dir = "ends")
  dev.off()
}
...
#imageWithText(as.matrix(top.mat), col.scale = list(heatmap.neg.col, heatmap.pos.col), 
  #split.at.vals = TRUE, grad.dir = "ends", show.text = FALSE)
```
...
```{r ad_interest_fig}
if(final.params){
  pdf(here("Results", "for_paper", "AD-related_expression.pdf"), width = 5, height = 7)
  for(i in 1:length(ad.interest)){
    gene.id <- gene.info[which(gene.info[,"external_gene_name" ]%in% ad.interest[i]), id_col]
    test <- plot_tx_with_genotype(expr.mat = scaled.expr, covar.table = covar.mat, 
...
if(!file.exists(mean.bd.val.file)){
  ...
  for(bd in 1:length(mouse.bd.genes)){
    ...
    #plot_individual_genes(mouse.bd.genes[[bd]][,1])
...
imageWithText(signif(bd.means[row.order,], 2), split.at.vals = TRUE, 
  col.scale = list(heatmap.neg.col, heatmap.pos.col), grad.dir = "ends", row.text.shift = 0.25,
  col.text.shift = 0.03)
par(mar = c(4,1,2,2))
...
imageWithText(signif(sbd.means[big.idx[row.order],], 2), split.at.vals = TRUE, 
  col.scale = list(heatmap.neg.col, heatmap.pos.col), grad.dir = "ends", row.text.shift = 0.15,
  col.text.shift = 0.05)
par(mar = c(4,1,2,2))
...
if(length(sig.path) > 1){
  ...
  imageWithText(mean.mean[sig.path[rev(row.order)],], show.text = FALSE, split.at.vals = TRUE,
    col.scale = list(heatmap.neg.col, heatmap.pos.col), grad.dir = "ends", col.text.rotation = 0,
  ...
  barplot(mean.mean[sig.path,], main = names(sig.path))
}
