...
gene.id <- mouse.gene.table[which(mouse.gene.table[,"external_gene_name"] == "Ide"), "ensembl_gene_id"]
gene.idx <- which(rownames(sub.count) == gene.id)
gene.counts <- sub.count[gene.idx,]
scaled.counts <- scale(gene.counts)
ordered.scaled <- scaled.counts[match(rownames(scaled.counts), condensed.table[,"animalName"])]
...
#treat everything except gene expression as a factor
info <- data.frame("sequencingBatch" = as.factor(condensed.table[,"sequencingBatch"]),
  ...
  "scaled_Ide_expression" = ordered.scaled) 
...
possible.covar <- c("sequencingBatch", "sex_ge", "age_batch", "ordered_geno", "scaled_Ide_expression")
...
mouse.path.ensembl.file <- file.path(general.processed.data.dir, paste0("Mouse_KEGG_", expression.type, "_for_GSEA.RDS"))
...
write.table(cbind(gene.names, mean.sig.expr, annot.col)[order(annot.col[,1]),], 
  file.path(results.dir, "DEG_clusters.txt"), sep = "\t", quote = FALSE)
```
...
mean.bd.val.file <- file.path(processed.data.dir, "mean_biodomain_expression.RDS")
...
mean.kegg.val.file <- file.path(results.dir, "mean_kegg_expression.RDS")
...
```{r plot_example_kegg, fig.height = 7, fig.width = 5}
if(group.representation == "means"){
