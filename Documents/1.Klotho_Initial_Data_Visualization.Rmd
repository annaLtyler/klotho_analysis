...
plotting.df <- data.frame("sequencingBatch" = as.factor(condensed.table[,"sequencingBatch"]),
  ...
  "age_batch" = as.numeric(condensed.table[,"age_batch"]),
  "climb_geno" = factor(condensed.table[,"climb_geno"], 
    levels = c("WT/WT", "FC/FC", "WT/FC", "WT/VS", "VS/VS")),
...
if(subgroup == "all ages"){
  ...
  #model <- lm(expr.decomp$u[,1]~as.factor(condensed.table[,"ordered_geno"])*as.factor(condensed.table[,"age_batch"]))
  model <- lm(expr.decomp$u[,1]~ordered_geno*age_batch, data = plotting.df)
...
plot_tx_with_genotype(expr.mat = scaled.expr, covar.table = plotting.df, 
...
test_full_model <- function(expr.mat, covar.mat){
  ...
  if(!file.exists(full.model.file)){
    ...
    diff.test <- lm(expr~(sex_ge+age_batch+ordered_geno)^2, data = df)
  ...
  if(length(sig.idx) > 0){
    pdf(file.path(results.dir, "age_geno_int.pdf"), width = 7, height = 5)
      for(i in 1:length(sig.idx)){
        ...
        model <- lm(adj.expr[,gene.id]~plotting.df[,"age_batch"]*plotting.df[,"ordered_geno"])
        #summary(model)
        #anova(model)
        #gene.id <- sample(rownames(factor.fdr), 1)
        gene.name <- gene.info[which(gene.info[,"ensembl_gene_id"] == gene.id),"external_gene_name"]
        interaction.plot(plotting.df[,"age_batch"], plotting.df[,"ordered_geno"], adj.expr[,gene.id], col = geno.cols, lwd = 3,
...
if(length(sex.geno.col) > 0){
  ...
  if(length(sig.idx) > 0){
    ...
    for(i in 1:length(sig.idx)){
      gene.id <- rownames(factor.fdr)[sig.idx[i]]
      model <- lm(adj.expr[,gene.id]~plotting.df[,"sex_ge"]*plotting.df[,"ordered_geno"])
      #summary(model)
      #anova(model)
      #gene.id <- sample(rownames(factor.fdr), 1)
      gene.name <- gene.info[which(gene.info[,"ensembl_gene_id"] == gene.id),"external_gene_name"]
      interaction.plot(plotting.df[,"sex_ge"], plotting.df[,"ordered_geno"], adj.expr[,gene.id], col = geno.cols, lwd = 3,
...
ad.names <- gene.info[match(plot.ad, gene.info[,"ensembl_gene_id"]),"external_gene_name"]
...
ordered.ind <- unlist(allele.idx[ordered.geno])
...
for(i in 1:length(ordered.geno)){
  end.idx <- start.idx + length(allele.idx[ordered.geno][[i]]) - edge.nudge
  draw.rectangle(start.idx, end.idx, 0+0.5, plot.dim[4]-0.5, border = "darkgray", lwd = 3)
  draw.rectangle(start.idx, end.idx, plot.dim[4]-0.5, plot.dim[4]+0.5, fill = geno.cols[ordered.geno[i]])  
  mid.point <- mean(c(start.idx, end.idx))
  text(x = mid.point, y = plot.dim[4], labels = ordered.geno[i])
...
```{r ad_mean_expr}
mean.ad <- sapply(allele.idx[ordered.geno], function(x) rowMeans(scaled.expr[ad.idx,x]))
...
mean.sig <- sapply(allele.idx[ordered.geno], function(x) rowMeans(scaled.expr[ad.sig,x]))
rownames(mean.sig) <- gene.info[match(rownames(mean.sig), gene.info[,"ensembl_gene_id"]), "external_gene_name"]
row.order <- order(plot.decomp(mean.sig, plot.results = FALSE)$u[,1])

png("~/Desktop/ad_means_sig.png", width = 10, height = 3, units = "in", res = 300)
par(bg = NA)
imageWithText(t(mean.sig[row.order,]), use.pheatmap.colors = TRUE, show.text = FALSE,
  row.text.shift = 0.02, row.text.adj = 1, col.text.shift = 0.13)
plot.dim <- par("usr")
for(i in 1:length(ordered.geno)){
...
for(bd in 1:length(mean.sbd.vals)){
  ...
  if(length(mean.sbd.vals[[bd]]) > 0){
    for(s in 1:length(mean.sbd.vals[[bd]])){
      ...
      diff.test <- aov.by.matrix(t(clustered.mat))
      diff.p <- summary(diff.test)[[1]]$"Pr(>F)"[1]
      vioplot(t(clustered.mat[ordered.geno,]), main = "", horizontal = TRUE, 
        las = 2, col = geno.cols[ordered.geno])
      segments(x0 = 0, y0 = 0, y1 = 5.7)
...
saveRDS(mouse.bd.k.jacc, file.path(general.processed.data.dir, "Mouse_KEGG_Intersection_Jaccard.RDS"))
...
saveRDS(human.bd.k.jacc, file.path(general.processed.data.dir, "Human_KEGG_Intersections_Jaccard.RDS"))
```
