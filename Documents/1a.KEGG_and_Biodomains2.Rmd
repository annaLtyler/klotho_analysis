...
test_effect <- function(values, test.factor, return.text = FALSE,
    ...
    plot.label = "", autoflip.stat.y = FALSE){
    ...
    linear.effect.p <- coefficients(summary(model))["test.factor.L","Pr(>|t|)"]
        ...
        if(plot.results){
            ...
            vioplot(values~test.factor, col = box.cols, xlab = "", main = plot.label, ylab = "Expression")
...
gene_vioplot <- function(gene.name, plot.label = "", 
    stat.x = 0.1, stat.y = 0.9, autoflip.stat.y = FALSE){
    ...
    test_effect(adj_expr[,gene.id], test.factor, 
...
```{r biodomains, fig.width = 10, fig.height = 5}
...
plot_individual_genes <- function(intersection.name = NULL, gene.list = NULL, 
    ...
    label.margin = 4, order.by = c("PC", "top.rank", "mean", "genotype", "none"), show.text = TRUE,
    legend.x = NULL, legend.y = NULL, stat.x = 0.1, stat.y = 0.9, 
...
for(i in 1:length(order.type)){
    png(paste0("~/Desktop/", "ordered_by_", order.type[i], ".png"), width = 10, height = 5, units = "in", res = 300)
    par(bg = NA)
    int.genes <- plot_individual_genes(intersection.name = term.name, 
        plot.type = "individual", order.by = order.type[i], stat.x = x[i], 
        stat.y = 0.2)
    dev.off()
}
...
for(i in 1:length(connector.names)){
    gene_vioplot(connector.names[connector.order[i]], autoflip.stat.y = TRUE)
}
#dev.off()
```
...
diff_cor_test <- function(gene.id, nperm = 1000, show.progress = FALSE){
    #which genes do we have expression for?
    common.genes <- intersect(gene.id, colnames(adj_expr))
    #get their names
    common.names <- mouse.genes[match(common.genes, mouse.genes[,"ensembl_gene_id"]), "external_gene_name"]
    ...
    gene.pairs <- pair.matrix(common.names, ordered = FALSE)
    ...
    for(i in 1:nrow(gene.pairs)){
        ...
        cor.diff <- diff(sapply(carrier.idx, function(x) cor(data1[x], data2[x])))
        rnd.cor.diff <- rep(NA, nperm)
        for(i in 1:nperm){
            rnd.factor <- sample(test.factor)
            rnd.idx <- lapply(carrier.geno, function(x) grep(x, rnd.factor))
            rnd.cor.diff[i] <- diff(sapply(rnd.idx, function(x) cor(data1[x], data2[x])))
        }
        #hist(rnd.cor.diff, xlim = c(-2, 2));abline(v = cor.diff)
        #par(mfrow = c(1,2));lapply(carrier.idx, function(x) plot.with.model(data1[x], data2[x], report = "cor.test"))
        emp.p <- one.sided.emp.p(cor.diff, rnd.cor.diff)

        pair.int.effect[gene1, gene2] <- cor.diff
        pair.int.p[gene1, gene2] <- min(emp.p)
    }
...
net_by_geno <- function(int_result, p.thresh = 0.001, plot.result = FALSE, 
    ...
    edge.width.factor = 5, title.level = "####"){
    ...
    if(plot.result){
        ...
        #cat(title.level, "FC\n")
        plot_net(net = fc.net, layout.mat = layout.mat, global.min = global.min, 
            global.max = global.max, vertex.size = vertex.size, 
            plot.label = "FC", edge.arrow.size = edge.arrow.size, 
            edge.width.factor = edge.width.factor)
        #cat("\n\n")
...
plot_net <- function(net, layout.mat = NULL, global.min = NULL, global.max = NULL,
    ...
    if(length(unique(sign(E(net)$weight))) == 1){ #if all edge weights are positive or negative
        if(all(sign(E(net)$weight) < 0)){
            col.scale <- "blue"
            grad.dir <- "low"
        }else{
            col.scale <- "brown"
        ...
        edge.col <- colors.from.values(E(net)$weight, split.at.vals = TRUE,
            col.scale = col.scale, grad.dir = grad.dir, light.dark = "f", 
...
directed_residuals <- function(gene1, gene2, plot.results = TRUE, report = "cor.test"){
    ...
    resid.forward <- resid(lm(adj_expr[,g2.id]~adj_expr[,g1.id]+test.factor))
    resid.reverse <- resid(lm(adj_expr[,g1.id]~adj_expr[,g2.id]+test.factor))
    ...
    if(plot.results){
        ...
        layout(layout.mat, widths = c(rep(0.8, 3), 1), heights = c(0.2, 1, 0.2, 1))
        ...
        for(g in 1:length(geno.idx)){
            ind.idx <- geno.idx[[g]]
            forward.stats[g,] <- plot.with.model(adj_expr[ind.idx,g1.id], resid.forward[ind.idx], col = geno.cols[g],
                xlab = gene1, ylab = paste(gene2, "residuals"), main = names(geno.cols)[g],
                report = "cor.test")
        }
        #reverse direction
        for(g in 1:length(geno.idx)){
            ind.idx <- geno.idx[[g]]
            reverse.stats[g,] <- plot.with.model(adj_expr[ind.idx,g2.id], resid.reverse[ind.idx], col = geno.cols[g],
                xlab = gene2, ylab = paste(gene1, "residuals"), main = names(geno.cols)[g], 
                report = "cor.test")
        }
        ...
        barplot(-log10(rbind(forward.stats[,2], reverse.stats[,2])), beside = TRUE, 
            col = rbind(geno.cols, geno.cols), density = c(NA, 50), 
            ylab = "-log10(p)", main = "-log10(p)")
        plot.dim <- par("usr")
        abline(h = 0) 

        #add labels for forward and reverse:
        par(mar = c(0,4,0,4))
        plot.text(paste("Forward Direction:", gene1, "->", gene2), cex = 2, font = 2)
        plot.text(paste("Reverse Direction:", gene2, "->", gene1), cex = 2, font = 2, x = 0.75)
...
```{r apoe_app}
#test.id <- bd.list$"APP Metabolism"
#test.id <- kegg.bd.list$"Lipid Metabolism"
test.id <- kegg.bd.list$"Synapse"
#length(test.id)
#barplot(sapply(test.id, length))

corr_result <- vector(mode = "list", length = length(test.id))
names(corr_result) <- names(test.id)
idx <- 1
for(i in 1:length(test.id)){
    print(i)    
    corr_result[[idx]] <- test_corr_int(test.id[[i]])
    idx <- idx + 1
}

#corr_result <- diff_cor_test(test.id, nperm = 1000, show.progress = TRUE)
pair.int.effect <- lapply(corr_result, function(x) x[[1]])
pair.int.p <- lapply(corr_result, function(x) x[[2]])
```

### Intersection p values {.tabset .tabset-fade .tabset-pills}
...
```{r qq, results = "asis"}
#pdf("~/Desktop/qq.pdf")
for(i in 1:length(pair.int.p)){
    cat("####", names(pair.int.p)[i], "\n")
    net.thresh = 10^(-3)
    qqunif.plot(as.vector(pair.int.p[[i]]), plot.label = names(pair.int.p)[i])
...
```{r kl_int, results = "asis"}

#pdf("~/Desktop/volcano.pdf")
for(i in 1:length(pair.int.effect)){
    cat("####", names(pair.int.effect)[i], "\n")
    plot(pair.int.effect[[i]], -log10(pair.int.p[[i]]), xlab = "Effect Size", ylab = "-log10(p)")
    abline(h = -log10(net.thresh))
    cat("\n\n")
}
#dev.off()
...
The barplot below shows how many of the total edges in each 
intersection were significantly differentially expressed across
genotypes.

```{r frac_sig}
sig.effect <- lapply(pair.int.p, function(x) which(x <= net.thresh, arr.ind = TRUE)); #significant
...
total.pairs <- sapply(pair.int.p, function(x) length(x) - ncol(x))
frac.sig <- sapply(1:length(sig.effect), function(x) nrow(sig.effect[[x]])/total.pairs[x])

sig.int <- which(frac.sig > 0)

pdf("~/Desktop/frac_sig.pdf", height = 28, width = 7)
par(mar = c(4, 20, 4, 2))
barplot(sort(frac.sig[sig.int]), horiz = TRUE, las = 2, cex.names = 0.7,
    main = "Intersections in\nSynapse with significantly\ndifferential correlations")
dev.off()
```

There were `r length(frac.sig)` KEGG-biodomain intersections 
in this biodomain, and `r length(sig.int)`
had significantly differentially correlated genes across genotypes.

### Directed networks {.tabset .tabset-fade .tabset-pills}

The interaction networks are directed because the interaction term
...
```{r dir_net, fig.width = 10, fig.height = 10, results = "asis"}
#pdf("~/Desktop/dir.net.pdf", width = 10, height = 10)
dir.nets <- net.layouts <- vector(mode = "list", length = length(corr_result))
names(dir.nets) <- names(net.layouts) <- names(corr_result)
for(i in sig.int){
    #get a layout for all networks
    dir.nets[[i]] <- corr_net(int_result = corr_result[[i]], 
        p.thresh = net.thresh, plot.result = FALSE)
    net.layouts[[i]] <- get_layout(dir.nets[[i]])

    plot_net(net = dir.nets[[i]], layout.mat = net.layouts[[i]],
        vertex.size = 1, plot.label = names(sig.int)[i])
...
```{r dir_test, fig.width = 11, fig.height = 7}
#gene1  <- "App"; gene2 <- "Bace1" #example of bi-directional interaction
...
directed_residuals(gene1, gene2)
```
...
that influence isn't really directed. So we can analyze
the network safely, but I don't think we should put too
much stock in the directionality.

The following bar plots show the in and out degree of each gene
in the network. App is the hub of the network, and primarily has
connections going out. This suggests that Klotho genotype 

```{r degree, fig.width = 7, fig.height = 15, eval = FALSE}
in.deg <- degree(dir.net, mode = "in")
out.deg <- degree(dir.net, mode = "out")
#head(sort(in.deg, decreasing = TRUE))
#head(sort(out.deg, decreasing = TRUE))

#pdf("~/Desktop/degree.pdf", width = 7, height = 15)
in.deg.order <- order(in.deg, decreasing = FALSE)
barplot(rbind(in.deg[in.deg.order], out.deg[in.deg.order]), beside = TRUE,
    horiz = TRUE, las = 2, col = c("#7fc97f", "#beaed4"), xlab = "Degree", cex.names = 0.5)
legend("bottomright", fill = c("#7fc97f", "#beaed4"), title = "Degree", 
    legend = c("in", "out"))
#dev.off()
```
...
edges in the two genotypes (FC carriers and VS carriers.) Overall,
the VS carriers seem to have stronger correlations.

```{r diff_net, fig.width = 10, fig.height = 10, results = "asis"}

#pdf("~/Desktop/geno_nets.pdf", width = 20, height = 10)
all.geno.nets <- vector(mode = "list", length = length(corr_net))
names(all.geno.nets) <- names(corr_net)
for(i in 1:length(sig.int)){
    cat("###", names(sig.int)[i], "\n")
    par(mfrow = c(1,2))
    all.geno.nets[[i]] <- net_by_geno(int_result = corr_result[[sig.int[i]]], 
        p.thresh = net.thresh, plot.result = TRUE, layout.mat = net.layouts[[sig.int[i]]], 
        title.level = "###")
    mtext(names(sig.int)[i], side = 3, outer = TRUE, line = -1.5, font = 2, cex = 1.5)
    cat("\n\n")
}
#dev.off()
...
#plot.enrichment(large.enrich, num.terms = 30)
```

What do these interactions look like up close? The plot below shows an example.

```{r app_int, fig.width = 10, fig.height = 7, eval = FALSE}
app.out <- direct_connect(dir.net, "App", mode = "out")
app.in <- direct_connect(dir.net, "App", mode = "in")
```
