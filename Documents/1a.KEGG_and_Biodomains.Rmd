...
gene insulin degrading enzyme (Ide) encoded on chromosome 19 at
37 Mb. This gene has been associated with AD before, and we want
to make sure that variation we see in this experiment is not related
to untracked variation in Ide. Here we look at Ide expression in our
mouse population, identify any correlated genes, and add Ide expression
as a covariate, if specified, in case it is having an effect on our results.
...
if(final.params){
    ...
    imageWithText(t(ad.tx.mean[row.order,]), show.text = FALSE, split.at.vals = TRUE,
        ...
        col.text.adj = 1, row.text.adj = 0.5)
    plot.dim <- par("usr")
    ...
    text(x = 1:length(short.ad.p), y = rep(plot.dim[4]+0.1, length(short.ad.p)), labels = sig.text[row.order], cex = 1.2)
    dev.off()
}
```
