...
needed.libraries <- c("pheatmap", "vioplot", "RColorBrewer",
  ...
  "wordcloud2", "baffle", "parallel", "foreach")
...
directed_residuals <- function(gene1, gene2, plot.results = TRUE, report = "cor.test"){
    ...
    if(plot.results){
        ...
        plot.text(paste("Reverse Direction:", gene2, "->", gene1, "\np =", reverse.int.p), cex = 2, font = 2, x = 0.75)
...
corr_result <- test_corr_int_parallel(sub.expr, test.factor, ncores = (detectCores()-1))
#end.par <- Sys.time()
#time.par <- end.par - start.par
#print(time.par)

#plot(corr_result1[[1]], corr_result[[1]])
#plot(-log10(corr_result1[[2]]), -log10(corr_result[[2]]))
...
There were `r total.pairs` total correlations among the `r ncol(pair.int.p)`
genes in this biodomain. At p value of `r 1/(10^net.thresh)`, there were 
...
plot_net(dir.net, layout.mat = net.layout,
    vertex.size = 1, plot.label = "APP Metabolism")
#dev.off()
```

```{r test_proj, eval = FALSE}
test <- bipartite_projection(dir.net)
```
...
#pdf("~/Desktop/geno_nets.pdf", width = 10, height = 10)
...
```{r app_int, fig.width = 10, fig.height = 7}
