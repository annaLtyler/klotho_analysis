...
plot_mean_bd_k <- function(mean.bd.k.mat, covar.mat, plot.label = "",
    ...
    if(length(has.vals) > 0){
        ...
        if(order.by == "top.rank"){
            ...
            plot.p <- threshold_p(ind.p[rownames(sub.mat)[max.idx],"model.p"], return.text = TRUE, sig.dig = 2)
        ...
        if(order.by == "means"){
            ...
            plot.p <- threshold_p(top.model["model.p"], return.text = TRUE)
...
gene_vioplot <- function(gene.name, plot.label = "", 
    ...
    gene.stats <- test_effect(adj_expr[ind.idx,gene.id], test.factor[ind.idx], 
        plot.result = plot.results, stat.x = stat.x, stat.y = stat.y,
        autoflip.stat.y = autoflip.stat.y, plot.label = gene.name,
        stat.y.spread = stat.y.spread, ylab = ylab, cex.lab = cex.lab,
        ylim = ylim, label.nudge = label.nudge)
...
term_vioplot <- function(outer.term, inner.term = NULL, mean.term.vals, 
    ...
    test.stats <- test_effect(unlist(term.expr), test.factor[unlist(geno.idx)], 
...
if(final.params){
    ...
    for(i in 1:length(fig.plots)){
       gene_vioplot(fig.plots[i], stat.y = stat.y[i], stat.x = stat.x[i], 
        cex.lab = 1.5, ylim = c(-2.8, 2.5), label.nudge = -0.1)
    }
    dev.off()
}
```
...
for(i in 1:length(to.plot)){
    ...
    for(j in 1:length(to.plot[[i]])){
        ...
        stats <- test_effect(values = sbd.vals, plot.factor = test.factor, plot.results = TRUE,
            ...
            stat.y.spread = 0.2, label.nudge = -0.1)
    }
}
dev.off()
```
...
par(mar = c(4,18,6,1))
imageWithText(mat = example.table, split.at.vals = TRUE, split.points = 0, 
    col.scale = list(heatmap.neg.col, heatmap.pos.col), grad.dir = "ends", 
    ...
    global.min = -0.3, global.max = 0.3,
    row.text.cex = 1.5, na.col = "white", class.mat = class.mat)

#draw some boundaries
plot.dim <- par("usr")
mid.pt <- plot.dim[2]*0.545
horiz.y <- segment_region(plot.dim[3], plot.dim[4], 6, "ends")
#these are a little off
horiz.y[3] <- horiz.y[3]+0.05
horiz.y[4] <- horiz.y[4]+0.1
draw.rectangle(min.x = plot.dim[1], max.x = mid.pt, 
    min.y = plot.dim[3], max.y = plot.dim[4], 
    border.col = "black", fill = NULL, lwd = 3)
segments(x0 = rep(mid.pt, length(horiz.y)), x1 = rep(plot.dim[2], length(horiz.y)),
    y0 = horiz.y)
vert.x <- segment_region(mid.pt, plot.dim[2], 4, "ends")
segments(x0 = vert.x,
    y0 = rep(plot.dim[3], length(vert.x)), y1 = rep(plot.dim[4], length(vert.x)))

#label the mean expression section
text(x = mean(c(plot.dim[1], mid.pt)), y = 0.3, labels = "Mean Expression (A.U.)", cex = 1.2)
#and the stats section
text(x = mean(c(plot.dim[2], mid.pt)), y = 0.3, labels = "Model Stats", cex = 1.2)
...
example.paths <- c("Oxidative phosphorylation", "Chemical carcinogenesis - reactive oxygen species")
...
for(k in 1:length(example.paths)){
    geno.vals <- lapply(geno.idx, function(x) mean.k.vals[example.paths[k],x])
    stats <- test_effect(mean.k.vals[example.paths[k],], test.factor,
        plot.result = TRUE, autoflip.stat.y = TRUE, label.nudge = -0.08,
...
if(final.params){
    ...
    test_effect(ind.vals, test.factor, plot.result = TRUE, autoflip.stat.y = TRUE)
    dev.off()
}
```
...
par(bg = NA)
plot_individual_genes(gene.list = int.genes, order.by = "means", stat.y = 0.8, stat.x = 0.05, stat.y.spread = 0.18, 
    label.nudge = -0.1, cex.lab = 1.2)
dev.off()
}
```
...
mapk.apoptosis.stats <- term_vioplot(outer.term = "MAPK signaling pathway", inner.term = "Apoptosis", 
    mean.term.vals = mean.bd.k.vals, stat.x = 0.05, stat.y = 0.2, 
    stat.y.spread = 0.12, autoflip.stat.y = FALSE, cex.lab = 1.2,
...
par(mfrow = c(1,2))
gene_vioplot("App", stat.y = 0.25, stat.y.spread = 0.18, label.nudge = -0.08)
gene_vioplot("Apoe", label.nudge = -0.08, stat.y = 0.9, stat.x = 0.22)
dev.off()
```
