...
rank.normalize <- TRUE #If TRUE, the expression values are rankZ normalized to reduce the effect of ourliers
```
...
stripchart(scaled.expr[ide.idx,]~covar.table$ordered_geno, col = "#dd1c77",
...
gene_vioplot <- function(gene.name, plot.label = "", 
    ...
    individuals = NULL, remove.outliers = FALSE){
...
directed_residuals <- function(gene1, gene2, plot.results = TRUE, report = "cor.test", 
    ...
    plot.geno.effect = FALSE, autoflip.stat.y = TRUE, stat.y.spread = 0.15,
    autoplace.text = FALSE, x.samples = 5, y.samples = 20, text.window.x = 2, 
    text.window.y = 4){
    ...
    test.df <- data.frame(adj_expr[,g1.id], adj_expr[,g2.id], test.factor)
    ...
    if(plot.geno.effect){
        ...
        test_effect(values = adj_expr[split.idx$low,g2.id], 
            plot.factor = test.factor[split.idx$low],
        ...
        test_effect(values = adj_expr[split.idx$high,g1.id], 
            plot.factor = test.factor[split.idx$high],
            plot.results = TRUE, stat.x = stat.x, stat.y = stat.y, 
...
#pdf("~/Desktop/dir.net.pdf", width = 20, height = 20)
...
barplot(top.deg, las = 2, names = hub.labels)
```
...
for(i in 1:length(all.response)){
    ...
    if(all(sign(all.response[[i]]) == 1)){
        imageWithText(all.response[[i]][row.order,], main = hub.names[i],
            col.scale = "brown", row.text.shift = 0.5, cex = 1, col.text.rotation = 0, 
            col.text.adj = 0.5, col.text.shift = 0.1, grad.dir = "high")
        }
    if(all(sign(all.response[[i]]) == -1)){
        imageWithText(all.response[[i]][row.order,], main = hub.names[i],
            col.scale = "blue", row.text.shift = 0.5, cex = 1, col.text.rotation = 0, 
    ...
    if(length(unique(as.vector(sign(all.response[[i]])))) > 1){
        imageWithText(all.response[[i]][row.order,], main = hub.names[i], split.at.vals = TRUE,
            col.scale = c("blue", "brown"), global.color.scale = TRUE, global.min = min.effect,
...
pair.stats <- apply(elist, 1, function(x) directed_residuals(x[1], x[2], plot.results = FALSE))
...
boxplot(pos.cor.stats, main = "Correlations by genotype for positive interactions")
abline(h = 0)

neg.cor.stats <- t(sapply(pair.stats[neg.weight.idx], function(x) x$forward.stats[,"r"]))
boxplot(neg.cor.stats, main = "Correlations by genotype for negative interactions")
abline(h = 0)

#both positive and negative interaction networks are enriched for
#ribosome and synapse
#pos.net.enrich <- gost(unique(as.vector(elist[pos.weight.idx,])), organism = "mmusculus")
#neg.net.enrich <- gost(unique(as.vector(elist[neg.weight.idx,])), organism = "mmusculus")
#plot.enrichment(pos.net.enrich, num.terms = 30, max.term.size = 3000)
#plot.enrichment(neg.net.enrich, num.terms = 30, max.term.size = 3000)
```


```{r pos_neg_net, eval = FALSE}
...
text(pos.deg.sum[to.label], neg.deg.sum[to.label], labels = rownames(adj)[to.label], 
    pos = 4, cex = 0.7)
...
#pdf("~/Desktop/degree_comp.pdf", width = 5, height = 12)
par(mar = c(4,6,4,2))
barplot(all.deg[,to.label[max.order]], beside = TRUE, 
...
to.plot <- which(abs(total.deg) > 2)
#length(to.plot) 
barplot(sort(total.deg[to.plot]), las = 2, main = "Total weighted degree")
abline(h = 0)
