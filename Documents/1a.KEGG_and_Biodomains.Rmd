...
adjp <- p.adjust(unlist(lapply(sbd.result, function(x) if(length(x) > 1){x$row.p}else{NA})), method = "fdr")
...
#draw on rectangles
highlight.examples <- FALSE
if(highlight.examples){
...
imageWithTextColorbar(signif(k.mean.mat, 2), split.at.vals = TRUE, 
    col.scale = list(heatmap.neg.col, heatmap.pos.col), grad.dir = "ends", cex = 1.2)
plot.dim <- par("usr")
plot.width <- plot.dim[2] - plot.dim[1]
text(x = (plot.dim[2]+(plot.width*0.4)), y = mean(plot.dim[3:4]), 
    labels = "Expression (A.U.)", srt = 270, cex = 1.2)
...
if(select_by == "R2"){
    ...
    sig.idx <- which(p.adjust(sub.val.result$row.p, "fdr") <= fdr.thresh)
    #length(sig.idx)
}
if(select_by == "PC"){
...
if(final.params){
    ...
    par(mar = c(4,1,2,2))
    effect.idx <- rev(match(rownames(geno.means[smaller.set,]), names(edge.weights)))
...
```{r biadjacency}
#intersection.fdr.thresh = 0.05;plot.width = 25
#intersection.fdr.thresh = 0.01;plot.width = 18
intersection.fdr.thresh = 0.001;plot.width = 12

intersect.sig.idx <- which(p.adjust(sub.val.result$row.p, "fdr") <= intersection.fdr.thresh)
...
#length(int.genes)
#plot_individual_genes(gene.list = int.genes, order.by = "means", plot.enrich = TRUE)
...
term_vioplot(outer.term = "MAPK signaling pathway", inner.term = "Apoptosis", 
    ...
    stat.y.spread = 0.12, autoflip.stat.y = FALSE, cex.lab = 1.5)
...
term_vioplot(outer.term = "MAPK signaling pathway", inner.term = "DNA Repair", 
    ...
    autoflip.stat.y = FALSE, stat.y.spread = 0.12, cex.lab = 1.5)
dev.off()
...
for(i in 1:length(hub.names)){
    ...
    gene.stats <- plot_individual_genes(gene.list = c(hub.id, connected.id[[i]]), 
        plot.label = paste("genes connected to", hub.names[i]), plot.enrich = TRUE,
...
#gene.id <- mouse.genes[which(mouse.genes[,"external_gene_name"] == gene.name), "ensembl_gene_id"]
plot_individual_genes(gene.list = gene.id[,"id"], order.by = "means")
```