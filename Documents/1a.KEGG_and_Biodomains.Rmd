...
```{r ide_dist}
vioplot(scaled.expr[ide.idx,]~covar.table$carrier_status, col = geno.cols,
    xlab = "genotype", ylab = "Scaled Ide expression")
stripchart(scaled.expr[ide.idx,]~covar.table$carrier_status, col = "#dd1c77",
...
geno.call <- sapply(animal.id, function(x) covar.table[which(rownames(covar.table) == x),"carrier_status"])
raw.ide.idx <- which(raw.expr[,"gene_id"] == ide.id)
raw.ide <- unlist(raw.expr[raw.ide.idx, match(animal.id, gsub("X", "", colnames(raw.expr)))])
ide.table <- cbind("animalID" = animal.id, "carrier_status" = geno.call, 
    "normalized_Ide_expr" = expr.level, "Ide_expr" = raw.ide, "Ide_class" = expr.class)
write.table(ide.table, "~/Desktop/Ide_expression.csv", sep = ",", row.names = FALSE)
...
#add Ide as a covariate. This has a new variant in B6 that causes
#high variation in gene expression. It is also an AD gene, so may
#be relevant here.
ordered.ide <- scaled.expr[ide.idx, match(rownames(covar.table), colnames(scaled.expr))]
covar.table$Ide <- ordered.ide
```

We adjusted the expression by non-age covariates.

```{r adjust}
dummy.covar <- dummy_covar(covar.table[,c("sequencingBatch", "sex_ge", "Ide")])
