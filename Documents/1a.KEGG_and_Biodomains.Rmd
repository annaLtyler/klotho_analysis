...
if(final.params){
    fig.plots <- c("Kl", "App", "Apoe")
    stat.y = rep(0.2, 3)
    stat.x <- c(0.1, 0.1, 0.75)
    pdf(here("Results", "for_paper", "AD-related_expression.pdf"), width = 5, height = 5)
...
if(final.params){
    ...
    png(here("Results", "for_paper", "Intersections_Top.png"), width = 6, height = 4, units = "in", res = 300)
    layout(matrix(c(1,2), nrow = 1), widths = c(1, 0.7))
    ...
    par(mar = c(4,1,2,2))
    barplot(sig.decomp$u[rev(smaller.set),1], horiz = TRUE)
...
```{r nets, fig.width = 25, fig.height = 25, results = "asis"}
...
full.bimat <- build_biadjacency(edge.names = rownames(sub.vals)[sig.idx], e.weights = edge.weights[sig.idx])
cl.bimat <- build_biadjacency(edge.names = rownames(sub.vals)[sig.idx], e.weights = sub.val.result$decomp$v[sig.idx,1]) #use PC vals to cluster
...
png("~/Desktop/biadjacency.png", height = 6, width = 12, units = "in", res = 300)
...
if(margin.plot == "degree"){
    barplot(rbind(kegg.neg.deg[col.order], kegg.pos.deg[col.order]), 
        ylab = "Number of interactions", las = 2, axisnames = FALSE, 
...
if(margin.plot == "degree"){
    barplot(rbind(bd.neg.deg[rev(row.order)], bd.pos.deg[rev(row.order)]), 
        horiz = TRUE, xlab = "Number of interactions", axisnames = FALSE,
        col = c(neg.col[dark.deg], pos.col[dark.deg]), density = density)
}else{
    plot.new()
...
#bd.names <- c("Synapse", "Mitochondrial Metabolism", "Proteostasis", "Immune Response", "Structural Stabilization"); kegg.names = c("Ribosome")
...
#gene.net <- go_gene_net(enrich, max.term.size = 3000, plot.results = TRUE, max.pval = 1e-6, min.vertex.size = 2, max.vertex.size = 8)
#dev.off()
