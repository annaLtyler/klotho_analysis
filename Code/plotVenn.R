...
plotVenn <- function(Vlist, cat.names = NULL, lwd = 3, cex = 2, label.cex = 2){
	...
	if(length(Vlist) == 3){
		...
		results <- list(sig12, sig13, sig23, sig123)
		names(results) <- c(
			paste(cat.names[c(1,2)], collapse = "-"), 
			paste(cat.names[c(1,3)], collapse = "-"),
			paste(cat.names[c(2,3)], collapse = "-"),
			paste(cat.names[c(1,2,3)], collapse = "-")
			)
		invisible(results)
		}
